...
const AllProductsScreen = ({ route, navigation }) => {
    ...
    useEffect(() => {
        ...
        startTrace('all_products_search').then((trace) => {
            if (cancelled) {
                stopTrace(trace, {
                    outcome: 'cancelled',
                });
            ...
            performanceTraceRef.current = trace;
        });
    ...
    useEffect(() => {
        if (
            !loading &&
            !hasStoppedTrace.current &&
            performanceTraceRef.current
        ) {
            hasStoppedTrace.current = true;

            stopTrace(performanceTraceRef.current, {
                outcome: error ? 'error' : 'success',
                resultCount: filteredPlants.length,
                category: activeChip,
            });
        }
    }, [loading, error, filteredPlants.length, activeChip]);
